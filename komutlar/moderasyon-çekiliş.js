...
exports.run = async (client, message, args) => {
   if(!message.member.hasPermission("ADMINISTRATOR")) return message.channel.send("Bu Komutu Kullanabilmek İçin **YÖNETİCİ** Yetkisine Sahip Olman Gerek.")
    ...
    if (!args[0]) {
        message.delete()
        return message.reply('Çekiliş ne kadar sürecek? (`1 saniye`, `1 dakika`, `1 saat`, `1 gün` şeklinde yazınız)').then(msg => msg.delete({timeout: 30000}))
    }
    if (isNaN(args[0])) {
        message.delete()
        return message.reply('Çekiliş ne kadar sürecek? (`1 saniye`, `1 dakika`, `1 saat`, `1 gün` şeklinde yazınız)').then(msg => msg.delete({timeout: 30000}))
    ...
    if (!args[2]) {
        message.delete()
        return message.reply('Çekilişte kaç kazanan olacak?').then(msg => msg.delete({timeout: 30000}))    
    }
    if (isNaN(args[2])) {
        message.delete()
        return message.reply('Çekilişte kaç kazanan olacak?').then(msg => msg.delete({timeout: 30000}))    
    ...
    if (!args.slice(3).join(' ')) {
        message.delete()
        return message.reply('Çekilişte ne verilecek?').then(msg => msg.delete({timeout: 30000}))    
    }
