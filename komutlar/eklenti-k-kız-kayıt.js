...
exports.run = async (client, message, args) => {
  ...
  if (!kızrol)
    return message.channel.send(
      `Sunucuda Kız Rolü Ayarlanmadığı İçin Komut Kullanılamaz ! `
    );

  let member = message.mentions.members.first();
  if (!member)
    return message.channel.send(
      `Kız Olarak Kayıt Edeceğin Kullanıcıyı Belirtmelisin ! `
    );
  let isim = args[1];
  ...
  const kayıtsistemi = new discord.MessageEmbed()
    ...
    .setColor("RED")
    .setDescription(
      `Kız Olarak Kayıt Edilen Kullanıcı: ${member} \n Kız Olarak Kayıt Eden Yetkili: <@!${
        message.author.id
      }> \n Kız Olarak Kayıt Eden Kullanıcının Kayıt Sayısı: **${
